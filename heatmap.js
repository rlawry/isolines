...
TemperatureMap.hslToGrayscaleRgb = function (ha, se, le) {
        // Convert HSL to RGB
        let rgb;
        rgb = this.hslToRgb(ha, se, le);
...
TemperatureMap.prototype.getColor = function (levels, value) {
    ...
    return TemperatureMap.hslToGrayscaleRgb(tmp, 1, 0.5);
};
...
TemperatureMap.prototype.setRandomPoints = function (points, width, height) {
    ...
    for (counter = 0; counter < values.length; counter++) {
        ...
        x = positions[counter][0];
        y = positions[counter][1];

        console.log(x + " , " + y);
        let randomValue = Math.floor(Math.random() * 31) + 30;

        if (Math.random() > 0.5) { v = -v; }
        if (Math.random() > 0.5) { v = v + 30; }

        rst.push({ x: x, y: y, value: randomValue });
    }
